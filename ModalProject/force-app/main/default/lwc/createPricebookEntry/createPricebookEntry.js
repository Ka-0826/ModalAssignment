...
import getCategoryOptions from '@salesforce/apex/PricebookEntryController.getCategoryOptions';
import createItems from '@salesforce/apex/PricebookEntryController.createItems';
import { getRecord } from 'lightning/uiRecordApi';

const PRICEBOOK_FIELDS = ['Pricebook__c.Id'];

export default class createPricebookEntry extends LightningElement {
    ...
    columnsAdd = [
        ...
        { label: '原価', fieldName: 'CostPrice', type: 'currency', editable: true },
    ...
    @wire(getRecord, { recordId: '$recordId', fields: PRICEBOOK_FIELDS })
    wiredPricebook({ data, error }) {
        if (data) {
            console.log('data:', data); //デバッグ用
            this.pricebookId = data.fields.Id.value;
            console.log('取得した pricebookId:', this.pricebookId); //デバッグ用
            
            if (this.pricebookId) {
                this.loadProducts();
                this.loadCategoryOptions();  // 商品分類をロード
            } else {
                console.error('pricebookId が設定されていません');
            }
        } else if (error) {
            console.log('wiredPricebook error:', error);
    ...
    async loadProducts() {
        try {
            const data = await getPricebookEntries({ pricebookId: this.pricebookId });
            console.log('data:', data); // デバッグ用
            ...
            const records = Array.isArray(data) ? data : data.records;
                ...
                console.log('最終的な gridData', JSON.parse(JSON.stringify(this.gridData))); //デバッグ用
    ...
    async loadCategoryOptions() {
        try {
            ...
            if (data) {
                ...
                console.log('最終的な options1', JSON.parse(JSON.stringify(this.options1))); //デバッグ用
                console.log('最終的な options2', JSON.parse(JSON.stringify(this.options2))); //デバッグ用
            }
        } catch (error) {
            console.error('カテゴリオプションの取得エラー:', error);
        }
    }
    ...
    handleChange1(event) {
        ...
        console.log('selectedOption1 更新後:', this.selectedOption1);
    }

    // 商品分類2（combobox2）の選択変更
    handleChange2(event) {
        this.selectedOption2 = event.detail.value;
        console.log('selectedOption2 更新後:', this.selectedOption2);
    }
    ...
    handleRowSelection(event) {
        ...
        console.log('選択された行:', JSON.stringify(this.selectedRows, null, 2));
    }
    ...
    handleNext() {
        ...
        console.log('選択されたデータ（次へボタン）:', JSON.parse(JSON.stringify(this.secondTableData)));
    ...
    handleCellChange(event) {
        ...
        // 変更されたデータをログ出力
        console.log('編集されたデータ:', JSON.parse(JSON.stringify(updatedValues)));
    
        updatedValues.forEach(update => {
            let row = this.secondTableData.find(item => item.Id === update.Id);
            if (row) {
                Object.keys(update).forEach(key => {
                    if (key === 'CostPrice' || key === 'UnitPrice') {
                        // 数値変換（整数または小数）
                ...
                console.log('更新後の行データ:', JSON.parse(JSON.stringify(row)));
            }
        });
    ...
    handleSave(event) {
        ...
        const finalData = this.secondTableData.map(item => ({
            Pricebook__c: this.recordId,
            Product2__c: item.Id,
            CostPrice__c: item.CostPrice != null && item.CostPrice !== '' ? item.CostPrice : 10,
            UnitPrice__c: item.UnitPrice != null && item.UnitPrice !== '' ? item.UnitPrice : 100,
            IsActive__c: true,
            Category1: item.Category1__c,
            Category2: item.Category2__c
        }));
        ...
        createItems({ finalData })
            .then(() => {
                this.showToast('成功', '商品が作成されました。', 'success');
            ...
            .catch(error => {
                ...
                this.showToast('エラー', '商品の作成に失敗しました。', 'error');
            });
    }

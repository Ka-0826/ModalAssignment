...
export default class createQuoteLineItem extends LightningElement {
    ...
    quoteId;                      // 現在開いているレコードIdの格納先
    wiredRecord;                  // refreshApex で使用するための変数
    data = [];                    // 取得した商品リスト
    error;
    ...
    wiredQuote(result) {
        // refreshApex に使用するために保存
        this.wiredRecord = result;

        if (result.data) {
            this.quoteId = result.data.fields.Id.value;
            
            if (this.quoteId) {
                this.loadProducts();
                this.loadCategoryOptions();  // 商品分類をロード
            } else {
                console.error('quoteId が設定されていません');
            }
    ...
    async loadProducts() {
        try {
            const data = await getQuotePricebookEntries({ quoteId: this.quoteId });
    ...
    async handleSave() {
        ...
        try {
            await createLineItems({ finalData })
            this.showToast('成功', '見積品目が作成されました。', 'success');
            ...
            this.showToast('エラー', '見積品目の作成に失敗しました。', 'error');
        }
    }
